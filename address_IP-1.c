...
void getData(char chain[100], int a, int b, int c, int d);
...
int main()
{
    int a, b, c,  d;
    char chaine[100];
    getData(chaine, a, b, c, d);
    condition(chaine , a, b, c, d);

    return 0;
}

void getData(char chaine[100], int a, int b, int c, int d)
{
...
void condition(char chaine[100], int a, int b, int c, int d)
{
    ...
    if (sscanf(chaine, "%d.%d.%d.%d", &a, &b, &c, &d)==4)
        {
            printf("%d.%d.%d.%d\n", a, b, c, d);
            if (a<x && b<e && c<e && d<e)
                {
